							...
							if (( $as > 1 )); then
								...
								if (( $agc > 1 )); then
									...
									while read xag; do
										...
										ah=$(ffprobe "$f" -show_streams -select_streams a:$ahc 2>&1 | grep channels= | sed s/channels=//g)
										if $dualaudio; then
											if [ "$ac" == "ac3" ]; then
												if (( $acc == 0 )); then
													if [[ "$ah" == "N/A" ]] || (( $ah > 2 )); then
								...
								ah=$(ffprobe "$f" -show_streams -select_streams a:0 2>&1 | grep channels= | sed s/channels=//g)
								if $dualaudio; then
									if [ "$ac" == "ac3" ]; then
										if [[ "$ah" == "N/A" ]] || (( $ah > 2 )); then
											dc="$dc -map $am -c:a:0 aac -ac:a:0 2 -ab:a:0 256k"
